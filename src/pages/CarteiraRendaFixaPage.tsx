...
export default function CarteiraRendaFixaPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const calendarioIpcaRecords = await fetchCalendarioIpcaBatch(rfProducts.filter(p => p.modalidade !== "Poupança"), dataCalculo);
                    ...
                    for (const row of chronRows) {
                      for (let m = 0; m < 12; m++) {
                        if (row.patrimonioMonths[m] !== null) {
                          barData.push({ mes: `${MONTH_LABELS[m]}/${String(row.year).slice(2)}`, patrimonio: row.patrimonioMonths[m]!, sortKey: `${row.year}-${String(m).padStart(2, "0")}` });
                        }
                      }
                    }
